...
/*   Updated: 2021/08/03 17:13:20 by lhoerger         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

void put_hexnbr(unsigned long long i, int is_capital)
{
	if ((i % 16) < 10)
	{
		i = (i % 16) + '0';
...
//Achtung großes X muss noch hinzugefügt werden
void ft_puthex(unsigned long long i, col *col1, int is_capital)
{
	//printf("%lli\n", i);
	col1->minus = 0; //entfernen
	//printf("Beginn: %i\n", i);
	if (i == 0)
	{
		col1->len = col1-> len + 1;
